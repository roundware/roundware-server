"""
Unit tests for audio normalization functionality in the speakers API
"""
...
class TestConvertAudio:
    ...
    def test_convert_uploaded_file_with_normalization(self, temp_dir, test_audio_file):
        """Test convert_uploaded_file when normalization is enabled"""
        filename = "test_audio.wav"
        test_audio_file(filename)
        
        with override_settings(AUDIO_NORMALIZATION_ENABLED=True):
            with patch('roundware.lib.convertaudio.normalize_audio_file') as mock_normalize:
                with patch('roundware.lib.convertaudio.convert_audio_file') as mock_convert:
                    result = convert_uploaded_file(filename)
...
class TestAudioNormalization:
    """Test cases for audio normalization functionality"""
...
class TestAudioNormalizationIntegration:
    ...
    def test_speakers_api_integration_enabled(self, temp_dir):
        """Test that speakers API calls normalization when enabled"""
        ...
        with override_settings(AUDIO_NORMALIZATION_ENABLED=True):
            with patch('roundware.lib.api.convertaudio.convert_uploaded_file') as mock_convert:
                ...
                # Should call convert_uploaded_file which includes normalization
